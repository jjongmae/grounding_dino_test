...
TEXT_THRESHOLD = 0.25
IOU_THRESHOLD = 0.8
VIDEO_PATH = r"D:\data\도로_비정형객체1\3.avi"
...
def draw(frame, boxes, labels, scores):
    ...
    for (x0, y0, x1, y1), lab, sc in zip(boxes, labels, scores):
        if not lab or lab.strip() == "" or lab == "road":
            continue
...
# ─────────── 이미지 4분할 함수 ─────────── #
def slice_image_into_quadrants(img):
    h, w = img.shape[:2]
    mid_h, mid_w = h // 2, w // 2
    return [
        (img[0:mid_h, 0:mid_w], 0, 0),             # 좌상
        (img[0:mid_h, mid_w:w], 0, mid_w),         # 우상
        (img[mid_h:h, 0:mid_w], mid_h, 0),         # 좌하
        (img[mid_h:h, mid_w:w], mid_h, mid_w),     # 우하
    ]
...
while cap.isOpened():
    ...
    # 슬라이스 4장으로 나누기
    for quadrant_img, offset_y, offset_x in slice_image_into_quadrants(frame):
        img_rgb = cv2.cvtColor(quadrant_img, cv2.COLOR_BGR2RGB)
        ...
        with torch.no_grad():
            for prompt in PROMPT_LINES:
                ...
                qh, qw = quadrant_img.shape[:2]
                boxes_xyxy = convert_boxes(raw_boxes, qw, qh)
                ...
                labels_all.extend(phrases)       # 이미 문자열 리스트
    ...
    # ──────── 시각화 ──────── #
    frame_vis = draw(frame, boxes_final, labels_final, scores_final)
